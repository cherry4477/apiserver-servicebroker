...
import (
	...
	"github.com/asiainfoldp/apiserver-servicebroker/pkg/client/informers_generated/externalversions"
...
func (si *SharedInformers) startInformers(shutdown <-chan struct{}) {
	go si.Factory.Prd().V1().BackingServices().Informer().Run(shutdown)
	go si.Factory.Prd().V1().BackingServiceInstances().Informer().Run(shutdown)
	go si.Factory.Prd().V1().ServiceBrokers().Informer().Run(shutdown)
}
